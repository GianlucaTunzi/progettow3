...
*/
function deleteOne(stringa,option){
  if(option){
...
function howManyDays(data){
  return Math.ceil(Math.abs(new Date()-data)/ (1000 * 60 * 60 * 24))
}
/* ESERCIZIO 10
  Scrivi una funzione chiamata "isTodayMyBirthday" che deve ritornare true se oggi è il tuo compleanno, falso negli altri casi.
*/
function isTodayMyBirthday(){
  const bd = new Date("11/04/2001")
...
*/
function addRedBackgroundToAnchor(){
  const a = document.querySelectorAll("a")
  a.forEach(e => e.style.backgroundColor = "red")
}
/* ESERCIZIO 24
  Scrivi una funzione per aggiungere un nuovo elemento alla lista non ordinata con id "myList".
*/
function addLiToMyList(){
